...
class Controller_Index extends Controller_Abstract {
	...
	public function init() {
		...
		$this->filters ['action'] ['index'] = array ('checkLogin' => 1, 'isChinaMobile' => 1 );
		$this->filters ['action'] ['test'] = array ('t' => 1 );
		$this->filters ['action'] ['context'] = array ('checkLogin' => 0, 'isChinaMobile' => 0 );
		Santa_Context::set ( 'user', array ('firstname' => 'ian', 'lastname' => 'shen' ) );
	}
	...
	public function index() {
		...
		$this->assign ( 'nums', array (0, 1, 2, 3, 4, 5 ) );
	...
	public function db() {
		...
		print_r ( $result );
		//使用select获取数据
		$result = $db->select ( 'test1', array ('where' => 'id=3' ) );
		print_r ( $result );
		//插入数据
		$result = $db->insert ( 'test1', array ('value' => 'xxx' ) );
		print_r ( $result );
		//更新数据
		$result = $db->update ( 'test1', array ('value' => 'xxxxy' ), 'id=10' );
		print_r ( $result );
		//删除数据
	...
	public function cache() {
		...
		print_r ( $mc->get ( 'haha' ) );
		$mc->set ( 'xxx', array ('id' => 3, 'name' => 'ian' ) );
		print_r ( $mc->get ( 'xxx' ) );
